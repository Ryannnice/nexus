...
import { getSceneState, layerPose, mix } from './timeline'
import { Backdrop, type StageView } from './Backdrop'
import { RecipeObjects } from './RecipeObjects'
...
function World(props: SceneProps) {
  ...
  useFrame(({ clock }, delta) => {
    const p = props.progress.current.value
    ...
    gl.domElement.dataset.sceneProgress = p.toFixed(4)
    ...
    motion.current = { spread: s.spread, reduced: props.reduced, focus: s.focus }

    const aspect = size.width / size.height
    // The camera now follows each ingredient. Other layers retain their vertical stack.
    const readingScale = mobile ? 1.02 : Math.min(1.3, aspect * 0.95 + 0.3)
    ...
    const worldHeight = mix(mix(heroHeight, libraryHeight, s.dock), tableHeight, s.table)
    const pitch = mix(mix(0.24, 0.51, s.dock), 0.82, s.table)
    const fov = mix(36, 18, s.table)
    ...
    const targetY = mix(
      mix(0, focusY.current - (mobile ? worldHeight * 0.13 : 0), s.dock),
      -0.9,
      s.table
    )
    const targetZ = 1.4 * readingScale * s.dock * (1 - s.table)
    camera.position.set(
      0,
    ...
    root.current!.position.set(
      mobile ? 0 : worldHeight * aspect * mix(0.105, -0.185, s.dock) * (1 - s.table),
      0,
      0
    )
    root.current!.rotation.y = turn.current.angle * s.table
    const heroY = mobile ? -0.5 : 0.05
    food.current!.position.y = mix(mix(heroY, 0, s.dock), 1.515, s.table)
    if (!props.reduced && s.dock < 0.01 && s.table < 0.01)
      food.current!.position.y += Math.sin(clock.elapsedTime * 0.7) * 0.018
    food.current!.scale.setScalar(mix(mix(heroScale, readingScale, s.dock), 2.25, s.table))
    food.current!.rotation.set(
      0,
      (props.reduced ? -0.18 : -0.18 + Math.sin(clock.elapsedTime * 0.13) * 0.2 + p * 0.9) *
        (1 - s.table),
      0
    )
    view.current = {
