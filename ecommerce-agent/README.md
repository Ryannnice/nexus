...
这些结果来自 100 个已知工具组成的合成闭集，证明了该实验条件下的分流、检索和工具链规划能力，但不等同于真实线上效果。当前项目没有评测新工具冷启动、真实流量表达漂移、工具参数生成、真实 API 执行成功率或多轮执行恢复。最终 `92.6222%` 表示静态路由与有序工具链选择成功率，不是端到端交易完成率。
