...
```text
用户请求
  → 多轮上下文管理（仅在需要时）：当前状态 + 最近 2 轮 + 更早记录 Top-2
  → 补全后的短请求；自包含请求保持原文不变
  → Qwen3-0.6B LoRA：电商 / 闲聊分流
  → 显式负样本训练的 Embedding + BM25：两路召回
  → 一级 RRF：N=40, vector_weight=0.8, rrf_k=5
  → bge-reranker-v2-m3：精排 Top-20
  → 二级 RRF：source_weight=0.4, rrf_k=8
  → 向 Planner 提供 Top-10
  → Qwen3-8B：77,724 条 SFT + RPO 主训练 + DPO 短程收敛
  → 有序工具链
  → 写回本轮记录、状态、摘要与更早记录
```

多轮层不修改任何模型权重，也不把整段历史直接塞给 Planner；唯一进入原单轮后端的是一条补全后的短请求。检索模块只向 Planner 提供经过 Cross-encoder 打分的候选，不存在“只精排 30 个，却让 Planner 读取 50 个”的未评分长尾。
...
| 多轮目标与引用恢复 | 封存测试中按标签定义的状态相关 128 轮 | **97.6563%**，即 **125 / 128** |
| 多轮状态相关轮次 | 独立封存测试 | 工具链全对率 **67.1875%**，即 **86 / 128** |
| 多轮全部轮次 | 独立封存测试 594 轮 | 工具链全对率 **77.1044%**，即 **458 / 594** |

同一 9K 口径下，8B Planner 完成监督训练时 Full Trace 为 `90.4556%`；偏好训练起点为 `90.5333%`，RPO 主训练与 DPO 短程收敛后达到 `92.6222%`。

多轮 v1 与 Legacy 6K 使用不同数据和任务口径，`77.10%` 不是原 Planner 从 `88.93%` 退化后的同集对比。多轮封存测试中，严格格式输出率和会话隔离守门均为 `100%`，自包含请求保持原文不变，Planner 输入 Token p95 为 `449`。
...
开发集固定原 Intent、检索和 Planner，只比较历史如何读取。A 不读历史，B 直接拼接近期原句，C 改用结构化状态和近期记录，D 在 C 上读取规则化滚动摘要；E 仍维护摘要，但在线远期读取改为按轮次或关键词查找独立记录。状态相关轮次的工具链全对率依次为 `39.06% / 50.78% / 64.06% / 69.53% / 71.09%`，最终锁定 `E-n2-k2-t640`。这组 A–E 数字只用于开发集选型；封存测试只评最终 E，不能把 `39.06% → 71.09%` 写成封存测试提升。

封存测试按首个失败环节统计：3 轮目标或引用恢复与标注不一致，32 轮在恢复一致后被固定检索漏掉必要工具，101 轮在恢复一致且必要工具全部进入 Top-10 后，固定 Planner 仍未得到完全一致的工具链。Badcase 与模板措辞问题有明显重合，但尚未做反事实实验量化影响。
...
- 多轮开发集与测试集各 594 轮、148 个会话；测试集在配置锁定后只打开一次。两边的具体原句和多工具组合没有重复，但复用同一批场景骨架。
- 多轮 v1 是由工具标签机械生成的短句压力集，不是自然对话基准。测试集中 6 个写操作工具涉及的 44 轮全部未达到工具链完全一致，尽管必要工具均已进入 Top-10；这提示动作表达需要人工复标与重写，不能直接当作可加回的 44 分。
...
- 模型外多轮框架图、A–E 开发集对比与一次封存测试；
- 训练、开发与冻结数据的隔离关系；
- 最终错误预算、效率数据与机器可读证据入口。

## 适用范围

这些结果来自 100 个已知工具组成的合成闭集，证明了该实验条件下的分流、检索、工具链规划和模型外目标恢复能力，但不等同于真实线上效果。当前项目没有评测新工具冷启动、真实流量表达漂移、工具参数生成、真实 API 执行成功率或执行失败后的恢复。专门的远期回忆场景只有 4 轮，另有 8 轮摘要压力；SQLite 也只用于本地实验。最终 `92.6222%` 表示单轮 9K 的联合分流与工具链成功率，多轮 `77.1044%` 表示 594 轮上的工具链全对率；两者都不是端到端交易完成率。
